...
from scripts.roadface.prepare_roadface_dataset import main as prepare_main
...
def write_plan(args: argparse.Namespace) -> Path:
    plan = {
        ...
        "notes": [
            ...
            "Depth models are evaluated through run_roadface_pipeline.py + evaluate_roadface_outputs.py.",
        ],
    }
...
def main() -> None:
    ...
    if args.prepare:
        old_argv = sys.argv[:]
        try:
            sys.argv = ["prepare_roadface_dataset"]
            prepare_main()
        finally:
            sys.argv = old_argv
    ...
    try:
        ...
        raise SystemExit(
            "Ultralytics is not installed. Run: uv sync --extra roadface"
        ) from exc
