...
def trip_list(args: argparse.Namespace) -> list[Path]:
    from scripts.roadface.roadface_lib import discover_trips, resolve_trip

    if args.trip.lower() == "all":
        return discover_trips(args.dataset)
    return [resolve_trip(args.trip, args.dataset)]


def image_paths_for_trip(trip_dir: Path, args: argparse.Namespace) -> list[Path]:
    from scripts.roadface.roadface_lib import find_image
...
def main() -> None:
    ...
    from scripts.roadface.roadface_lib import read_image
