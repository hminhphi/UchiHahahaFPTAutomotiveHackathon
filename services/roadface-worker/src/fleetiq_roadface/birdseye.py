...
def _perspective_matrices(
    ...
    destination_margin = float(
        np.clip(args.bird_destination_margin_ratio, 0.05, 0.42) * w
    )
...
def _local_peaks(
    histogram: np.ndarray, min_distance: int, max_count: int = 16
) -> list[int]:
    ...
    ordered = sorted(
        (int(x) for x in candidates), key=lambda x: float(histogram[x]), reverse=True
    )
    selected: list[int] = []
    for x in ordered:
...
def _seed_pair(
    ...
    for left_index in range(len(peaks) - 1):
        for right_index in range(left_index + 1, len(peaks)):
            ...
            candidates.append(
                (center_cost + 0.55 * width_cost - 0.025 * strength, left, right)
            )
    if candidates:
...
def _window_pixels(
    ...
    anchor_center = (
        float(np.median(x_all[anchor_indices]))
...
def _normalize_candidate(
    ...
    brackets_ego = (
        left_near - 0.12 * expected_width <= ego_x <= right_near + 0.12 * expected_width
    )
    width_valid = (
        np.all(widths > 0)
        and expected_width * (1.0 - args.bird_width_tolerance)
        <= width_mean
...
def _blend_fit(
    previous: np.ndarray, current: np.ndarray, alpha: float, h: int
) -> np.ndarray:
    alpha = float(np.clip(alpha, 0.0, 1.0))
    sample_y = np.linspace(0, h - 1, 48)
    blended_x = alpha * np.polyval(current, sample_y) + (1.0 - alpha) * np.polyval(
        previous, sample_y
    )
...
def detect_birdseye_lane(
    ...
    left_seed, right_seed, seed_status = _seed_pair(
        bird_mask, ego_x_bird, expected_width, args
    )
    left_fit, left_pixels, left_status = _fit_lane(
        bird_mask, left_seed, previous[0], args
    )
    right_fit, right_pixels, right_status = _fit_lane(
        bird_mask, right_seed, previous[1], args
    )
    ...
    if (
        tracker is not None
        and tracker.has_previous
        and "reuse_previous" not in candidate.status
    ):
    ...
    _anchor_x_image, anchor_y_image = _project_point(
        (anchor_x, 0.65 * (h - 1)), inverse
    )

    total_support = left_pixels[0].size + right_pixels[0].size
    support_score = min(
        1.0, total_support / max(2.0 * float(args.min_lane_pixels) * 5.0, 1.0)
    )
    ...
    width_stability = 1.0 - min(
        1.0, float(np.std(widths)) / max(0.20 * expected_width, 1.0)
    )
    confidence = float(
        np.clip(
            (0.18 + 0.52 * support_score + 0.30 * width_stability)
            * candidate.confidence_scale,
            0.05,
            0.98,
        )
    )
