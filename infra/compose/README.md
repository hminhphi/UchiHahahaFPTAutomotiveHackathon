...
| --- | --- | --- |
| `core` | PostgreSQL, Redis, MQTT, API, web, event gateway | Product and protocol development |
...
the binary camera WebSocket framing contract, and CarSky acknowledgement.
...
`data/` is mounted read-only at `/data`. Generated outputs go to `/artifacts`.
Neither directory is baked into images. PostgreSQL, Redis, and Mosquitto use
named volumes.
